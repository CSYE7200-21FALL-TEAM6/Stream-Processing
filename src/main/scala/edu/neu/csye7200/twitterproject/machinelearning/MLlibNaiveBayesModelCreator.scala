package edu.neu.csye7200.twitterproject.machinelearning

import edu.neu.csye7200.twitterproject.utils.{PropertiesLoader, SQLContextSingleton, StopWordsLoader}
 ...
 */
// spark-submit --class "org.roy.spark.sentiment.mllib.SparkNaiveBayesModelCreator" --master spark://spark:7077 spark-streaming-corenp-mllib-tweet-sentiment-assembly-0.1.jar
...
//    LogUtils.setLogLevels(sc)
    ...
    val stopWordsList = sc.broadcast(StopWordsLoader.loadStopWords(PropertiesLoader.nltkStopWordsFileName))
  ...
  def createAndSaveNBModel(sc: SparkContext, stopWordsList: Broadcast[List[String]]): Unit = {
    ...
    val labeledRDD = tweetsDF.select("polarity", "status").rdd.map {
      case Row(polarity: Int, tweet: String) =>
        val tweetInWords: Seq[String] = MLlibNaiveBayesPrediction.getBarebonesTweetText(tweet, stopWordsList.value)
  ...
  def validateAccuracyOfNBModel(sc: SparkContext, stopWordsList: Broadcast[List[String]]): Unit = {
    ...
    val actualVsPredictionRDD = tweetsDF.select("polarity", "status").rdd.map {
      case Row(polarity: Int, tweet: String) =>
        ...
        val tweetInWords: Seq[String] = MLlibNaiveBayesPrediction.getBarebonesTweetText(tweetText, stopWordsList.value)
        (polarity.toDouble,
  ...
  def saveAccuracy(sc: SparkContext, actualVsPredictionRDD: RDD[(Double, Double, String)]): Unit = {
    ...
    actualVsPredictionDF.coalesce(1).write
      ...
      .save(PropertiesLoader.naiveBayesModelAccuracyPath)
  }
